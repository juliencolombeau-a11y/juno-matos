...
export default defineEventHandler(async (event) => {
  ...
  if (session.user?.id === id && body.role !== 'admin') {
    throw createError({
      statusCode: 403,
      statusMessage: 'Vous ne pouvez pas vous retirer le rôle d’administrateur.',
    })
  }

  if (targetUser.role === 'admin' && body.role === 'editor') {
    ...
    if (total <= 0) {
      throw createError({
        statusCode: 409,
        statusMessage: 'Au moins un administrateur doit rester actif.',
      })
    }
  }
